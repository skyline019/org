...
JaCoCo 聚合报告：`org-coverage/target/site/jacoco-aggregate/index.html`（根目录 `verify` 要求 `com.skyline.org` 指令覆盖率 ≥ 80%、分支覆盖率 ≥ 62%，含 SpotBugs 静态分析与 OWASP dependency-check；与 CI 一致）
